...
Cardano Slips — an open specification + TypeScript SDK that turns a shareable URL into a signable Cardano transaction. The client **derives** exact transaction effects from the tx body, resolved inputs, user addresses, and protocol parameters. If derived effects contradict the endpoint's declared transaction intent, the client blocks signing. The verifier does not check the meaning of titles, descriptions, or messages. No registry, no custody, no relayer.

This file holds the shared working instructions for the whole repository. Keep
the rules here; other instruction entry points should refer to this file.

## Starting and finishing work

- Read `git status --short`, `git branch --show-current`, and the relevant diff first. Preserve existing edits and untracked files; never reset, clean, or stash them to make a task easier.
- Read the source-of-truth documents below and the relevant package's README, source, and tests before editing. The docs describe the delivery plan as well as implemented work; inspect the checkout before assuming a module or app exists.
- Use the GitHub CLI to read issues, dependencies, PRs, and checks. For example: `gh issue view <number> --repo emmanuel-musau/cardano-slips --json title,body,state` and `gh pr view <number> --repo emmanuel-musau/cardano-slips`. Check each `Depends on #N` issue before starting ticketed work; an old session note is not evidence that a dependency closed.
- Carry out explicitly requested local work without posting to GitHub. Local review and editing do not authorise comments, issue or board updates, commits, pushes, PRs, releases, or deployments.
- Use `.env.example` for configuration names. Do not read `.env` or other `.env.*` files containing credentials, print secrets, or copy them into docs or fixtures.
- Before handing back, review the diff, run the required checks, and report what changed, which checks passed or failed, and any remaining blocker. State what was not run; never present cached or partial results as a fresh full run.
- If work is interrupted, leave a concise handoff in the conversation: task, branch, changed files, checks run, and next step. Recheck the working tree and issue state when resuming.

## Attribution — non-negotiable

Keep repository output attributed to the human committer. Specifically:

- No tool co-author trailers in commits.
- No generated-by credits in PR bodies, issues, or release notes.
- No tool self-references in code comments, docs, changesets, or commit messages.
...
Run from the repository root. Use Node 22 or newer and the pnpm version pinned
in `package.json` under `packageManager`; do not replace the lockfile or switch
package managers.
...
pnpm typecheck        # source and test types across the workspace
pnpm test:repo        # root documentation and repository checks
pnpm exec vitest run --project repo test/verifier-signature.test.ts
pnpm exec turbo run test --filter=@cardano-slips/verifier
```

Use a focused test while iterating, then run `pnpm lint`, `pnpm typecheck`,
`pnpm test`, and `pnpm build` before proposing work as ready. Turbo builds
workspace dependencies for package tests. Markdown is deliberately excluded
from Prettier; review its text, links, and tables directly rather than reflowing
whole files. See `CONTRIBUTING.md` for setup and `docs/ARCHITECTURE.md` for
the build and test layout.
