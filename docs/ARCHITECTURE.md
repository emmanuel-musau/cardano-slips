...
| `parameters.ts` | The protocol parameters the engine is handed rather than fetches. The deposits so far; the minimum-fee coefficients, the per-byte cost and the slot-to-time mapping arrive with the tickets that need them. |
